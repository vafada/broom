...
// so that the config file can be shared between Broom and doom.exe
...
//
static bool SndDeviceInList(snddevice_t device,
                               const snddevice_t *list,
                               int len)
{
    for (int i = 0; i < len; ++i) {
        if (device == list[i]) {
...
//
static bool I_IsMusicDeviceSupported(const music_module_t* mus_module) {
    const snddevice_t* snd_devices = mus_module->sound_devices;
    int num_snd_devices = mus_module->num_sound_devices;
    return SndDeviceInList(snd_musicdevice, snd_devices, num_snd_devices);
}

#ifdef _WIN32
static bool I_CanUseMusicModule(const music_module_t* mus_module) {
    if (!I_IsMusicDeviceSupported(mus_module)) {
        // Music device not supported
        return false;
    }
    // On Windows, do not use the native Windows MIDI module
    // if Timidity is configured.
    return mus_module != &music_win_module
           || strcmp(timidity_cfg_path, "") == 0;
}
#else
static bool I_CanUseMusicModule(const music_module_t* mus_module) {
    return I_IsMusicDeviceSupported(mus_module);
}
#endif
...
static void InitMusicModule(void) {
    ...
    for (int i = 0; music_modules[i] != NULL; ++i) {
        const music_module_t* mus_module = music_modules[i];
        if (!I_CanUseMusicModule(mus_module)) {
            continue;
        }
        // Initialize the module
        if (mus_module->Init()) {
            music_module = mus_module;
            return;
        }
    }
}
...
void I_UpdateSound(void) {
    ...
    if (active_music_module && active_music_module->Poll) {
...
void I_PrecacheSounds(sfxinfo_t *sounds, int num_sounds) {
    if (sound_module && sound_module->CacheSounds) {
...
#ifdef HAVE_FLUIDSYNTH
static void I_BindFluidSynthSoundVariables() {
...
#ifdef _WIN32
static void I_BindWinSoundVariables() {
...
#ifdef _WIN32
    I_BindWinSoundVariables();
#endif

#ifdef HAVE_FLUIDSYNTH
    I_BindFluidSynthSoundVariables();
#endif
}
